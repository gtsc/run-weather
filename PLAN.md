...
**Key decisions:** Pure frontend (no backend), Open-Meteo API only (free, no key, CORS-friendly), Svelte 5 + Vite + Tailwind, localStorage for preferences.
...
run-weather/
  src/
    ...
    lib/
      api/
        geocoding.js            # Postcode/GPS → lat/lng via Open-Meteo
        forecast.js             # Fetch + reshape 7-day hourly forecast
      scoring/
        engine.js               # Score each hour 0-100
        windows.js              # Find best consecutive-hour run windows
        weatherCodes.js         # WMO code → label + penalty mapping
      stores/
        preferences.svelte.js   # User prefs, synced to localStorage
        weather.svelte.js       # Forecast data + derived scores
        location.svelte.js      # Current lat/lng/name
      utils/
        format.js               # Time/temp formatting helpers
    components/
...
- Configure `vite.config.js` with tailwindcss plugin (before svelte plugin)
- Set up `src/app.css` with `@import "tailwindcss"` and theme colours (green/amber/red)
- Verify dev server runs

#### Step 2 — API modules
- **`geocoding.js`**: Call `geocoding-api.open-meteo.com/v1/search?name={postcode}&countryCode=GB`. Fallback strategy: try full postcode → outward code → area letters
- **`forecast.js`**: Call `api.open-meteo.com/v1/forecast` with hourly params: `temperature_2m, apparent_temperature, precipitation_probability, precipitation, wind_speed_10m, wind_gusts_10m, weather_code, is_day`, 7 days, timezone `Europe/London`. Reshape parallel arrays into array of hour objects

#### Step 3 — Scoring engine
- **`weatherCodes.js`**: Map all WMO codes to `{ label, penalty }` (penalty 0-95)
- **`engine.js`**: `scoreHour(hour, prefs) → 0-100` with weights:
  ...
  - Darkness: 5 pts
- **`windows.js`**: `findRunWindows(dayHours, scores, durationHours)` — slide a window of N hours, compute avg score, filter out windows containing any hour < 20, return top 3 sorted by score

#### Step 4 — Stores
- **`location.svelte.js`**: Holds `{ latitude, longitude, name }`
- **`preferences.svelte.js`**: `rainTolerance` (0-1, default 0.3), `tempMin` (default 8), `tempMax` (default 15), `durationHours` (default 1). Load/save localStorage
- **`weather.svelte.js`**: On location change, fetch forecast. Derive scores from raw data + preferences (auto-recompute when prefs change)
