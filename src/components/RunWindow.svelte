<script lang="ts">
  ...
  const maxPrecipProb = $derived(Math.max(...window.hours.map((h) => h.precipProbability)));
  const maxWind = $derived(Math.max(...window.hours.map((h) => h.windSpeed)));
  const maxSnowDepth = $derived(Math.max(...window.hours.map((h) => h.snowDepth)));
...
<div
  class="flex items-center gap-3 p-3 bg-run-card rounded-xl border border-run-border hover:border-run-muted/40 transition-colors"
>
  <div
  ...
  <div class="flex-1 min-w-0">
    ...
    <div class="text-xs text-run-muted flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5">
      ...
      {#if maxSnowDepth > 0}
        <span>{Math.round(maxSnowDepth * 100)} cm snow</span>
      {/if}
    </div>
  </div>
</div>
